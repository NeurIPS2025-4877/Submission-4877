...
class TestUnit_NonSynth:
    def __init__(self, covariates, treatment_ids, treatment_propensities, true_outcomes):
        self.data_dict = {}
        self.add_covariates(covariates)
        self.add_treatments(treatment_ids, treatment_propensities)
        self.set_true_outcomes(true_outcomes)

    def add_covariates(self, covariates) -> None:
        self.data_dict["covariates"] = covariates

    def add_treatments(self, treatment_ids, treatment_propensities) -> None:
        assert len(treatment_ids) > 0
        assert len(treatment_propensities) > 0
        self.data_dict["treatment_ids"] = treatment_ids
        self.data_dict["treatment_ids_to_propensity_dict"] = dict(
            zip(treatment_ids, treatment_propensities)
        )
        self.data_dict["propensity_to_treatment_ids_dict"] = dict(
            zip(treatment_propensities, treatment_ids)
        )
        self.data_dict["propensities"] = sorted(treatment_propensities, reverse=True)
    ...
    def get_k_likely_treatment_ids(self, k: int) -> list:
        return [
            self.data_dict["propensity_to_treatment_ids_dict"][p]
            for p in self.data_dict["propensities"][:k]
        ]
    ...
    def get_treatment_combinations(self, k: int) -> list:
        k_likely_treatment_ids = self.get_k_likely_treatment_ids(k)
        return get_treatment_combinations(k_likely_treatment_ids)

    def evaluate_predictions(self, k: int) -> Tuple[float, float]:
        treatment_pairs = self.get_treatment_combinations(k)

        valid_pairs = []
        for t1, t2 in treatment_pairs:
            if (
                t1 in self.data_dict["true_outcomes"] and
                t2 in self.data_dict["true_outcomes"] and
                # "predicted_outcomes" in self.data_dict and
                t1 in self.data_dict["predicted_outcomes"] and
                t2 in self.data_dict["predicted_outcomes"]
            ):
        ...
        for t1, t2 in valid_pairs:
            y1, y2 = self.data_dict["true_outcomes"][t1], self.data_dict["true_outcomes"][t2]
            o1, o2 = self.data_dict["predicted_outcomes"][t1], self.data_dict["predicted_outcomes"][t2]
            p1 = self.data_dict["treatment_ids_to_propensity_dict"][t1]
            p2 = self.data_dict["treatment_ids_to_propensity_dict"][t2]

            true_effects.append(y1 - y2)
            pred_effects.append(o1 - o2)
            weights.append(p1 * p2)

        true_effects = np.array(true_effects)
        pred_effects = np.array(pred_effects)
        se = np.square(true_effects - pred_effects)

        return np.mean(se), np.average(se, weights=np.ravel(weights))
    
    # def get_predicted_causal_effects(self, k: int) -> np.ndarray:
    #     predicted_causal_effects = []
    #     for t1, t2 in self.get_treatment_combinations(k):
    #         if t1 not in self.data_dict["predicted_outcomes"] or t2 not in self.data_dict["predicted_outcomes"]:
    #             continue
    #         o1, o2 = self.data_dict["predicted_outcomes"][t1], self.data_dict["predicted_outcomes"][t2]
    #         predicted_causal_effects.append(o1 - o2)
    #     return np.array(predicted_causal_effects)
    ...
    #     weights = []
    #     for t1, t2 in self.get_treatment_combinations(k):
    #         if t1 not in self.data_dict["true_outcomes"] or t2 not in self.data_dict["true_outcomes"]:
    #             continue
    #         y1, y2 = self.data_dict["true_outcomes"][t1], self.data_dict["true_outcomes"][t2]
    #         p1 = self.data_dict["treatment_ids_to_propensity_dict"][t1]
    #         p2 = self.data_dict["treatment_ids_to_propensity_dict"][t2]
    #         true_causal_effects.append(y1 - y2)
    #         weights.append(p1 * p2)
    #     return np.array(true_causal_effects), weights

    # def evaluate_predictions(self, k: int) -> Tuple[float, float]:
    #     true_effects, weights = self.get_true_causal_effects(k)
    #     pred_effects = self.get_predicted_causal_effects(k)
    #     if len(true_effects) == 0:
    #         return np.nan, np.nan
    #     se = np.square(true_effects - pred_effects)
    #     return np.mean(se), np.average(se, weights=weights)