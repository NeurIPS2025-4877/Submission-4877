...
def read_csv():
    properties = {}
    with open("./dataset/TCGA/gdb9.sdf.csv", "r") as f:
...
def fetch_all_raw_data(num_graphs: int = 500):
    ...
    raw_data = []
    fetch_qm9_data()
    all_files = list(Chem.SDMolSupplier("./dataset/TCGA/gdb9.sdf", False, False, False))
...
def fetch_qm9_data():
    download_path = "./dataset/TCGA/gdb9.tar.gz"
...
def get_TCGA_pca_data(n_components: int = 10):
    pca_path = "./dataset/TCGA/tcga_pca.p"
...
def fetch_TCGA_data(path="./dataset/TCGA/tcga.p"):
...
def get_TCGA_unit_features(path="./dataset/TCGA/tcga.p"):
